...
import { Check, X, Loader2, ArrowRight, ShieldCheck } from 'lucide-react';
...
export function Plans() {
    ...
    const handleContratar = (plano, precoObj) => {
        const whatsappNumber = '5511987654221'; // Seu número do WhatsApp
        const valorFormatado = precoObj?.valor
            ? `R$ ${precoObj.valor.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}`
            : '';

        const mensagem = `Olá! Tenho interesse no plano *${plano.nome}* (${periodicidade}) no valor de ${valorFormatado}. Gostaria de mais informações sobre como contratar!`;
                    ...
                    {plans.map((plano) => {
                        ...
                        return (
                            ...
                            >
                                ...
                                {/* Botão de Ação Direto para Contratação */}
                                <div>
                                    <button
                                        onClick={() => handleContratar(plano, precoObj)}
